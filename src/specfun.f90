...
!                         by L=j+1+[k*(k+1)]/2; j,k=0,1,...,Km
      ...
      do k = 1 , Km - 1
         do j = 1 , k
            l3 = k*(k+1)/2 + j + 1
            l4 = (k+1)*(k+2)/2 + j + 1
            a(l4) = (j+0.5_wp*k+0.125d0/(2.0*j+k+1.0))*a(l3)             &
                  & - (j+0.5_wp*k-1.0+0.625d0/(2.0*j+k+1.0))*a(l3-1)
         enddo
      enddo
      end
...
!       Output:  PV --- Vv(x)
      ...
      Pv = 1.0_wp
      do k = 1 , 18
         r = 0.5_wp*r*(2.0*k+Va-1.0)*(2.0*k+Va)/(k*x*x)
         Pv = Pv + r
...
!                NM --- Highest order computed
      ...
      enddo
      if ( x<=8.0_wp ) then
         Bk(0) = -(log(0.5_wp*x)+el)*Bi(0) + s0*sk0
...
!       Output:  CDN --- Dn(z)
      ...
      Cdn = (1.0_wp,0.0_wp)
      do k = 1 , 16
         cr = -0.5_wp*cr*(2.0*k-n-1.0)*(2.0*k-n-2.0)/(k*z*z)
         Cdn = Cdn + cr
...
!       Output:  PD --- Dv(x)
      ...
      Pd = 1.0_wp
      do k = 1 , 16
         r = -0.5_wp*r*(2.0*k-Va-1.0)*(2.0*k-Va-2.0)/(k*x*x)
         Pd = Pd + r
...
!                VK2 --- K2/3(x)
      ...
      if ( x<=12.0 ) then
         ...
         do l = 1 , 2
            ...
            enddo
            qx = 0.125d0*(vv-1.0)*qx/x
            xk = x - (0.5_wp*l/3.0_wp+0.25d0)*pi
            a0 = sqrt(rp2/x)
            ck = cos(xk)
            sk = sin(xk)
            if ( l==1 ) then
...
!                NM --- Highest order computed
      ...
      enddo
      if ( a0<=9.0 ) then
         Cbk(0) = -(log(0.5_wp*z1)+el)*Cbi(0) + cs0*csk0
