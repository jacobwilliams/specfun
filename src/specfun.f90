...
!  c and a large cx
      ...
      sw = 0.0_wp
      do k = 2 , nm
         r = r*(m+k-1.0)*(m+k+ip-1.5_wp)/(k-1.0_wp)/(k+ip-1.5_wp)
         suc = suc + r*Df(k)
...
!       Output:  PS --- psi(x)
      ...
      elseif ( xa+.5==int(xa+.5) ) then
         ...
         enddo
         Ps = -gamma + 2.0_wp*s - 1.386294361119891d0
      else
         if ( xa<10.0 ) then
            n = 10 - int(xa)
            do k = 0 , n - 1
...
!                SI --- Si(x)
      ...
      elseif ( x<=32.0_wp ) then
         ...
         xg1 = bj(1)
         do k = 2 , m
            xr = .25_wp*xr*(2.0*k-3.0)**2/((k-1.0)*(2.0*k-1.0)**2)*x
            xg1 = xg1 + bj(k)*xr
         enddo
         xr = 1.0_wp
         xg2 = bj(1)
         do k = 2 , m
            xr = .25_wp*xr*(2.0*k-5.0)**2/((k-1.0)*(2.0*k-3.0)**2)*x
            xg2 = xg2 + bj(k)*xr
         enddo
         xcs = cos(x/2.0_wp)
         xss = sin(x/2.0_wp)
...
!                         by L=j+1+[k*(k+1)]/2; j,k=0,1,...,Km
      ...
      do k = 0 , Km - 1
         ...
         g = -(1.5_wp*k+0.625d0/(3.0*(k+1.0_wp)))*g0
         a(l1) = f
         a(l2) = g
         f0 = f
         g0 = g
      ...
      do k = 1 , Km - 1
         do j = 1 , k
            l3 = k*(k+1)/2 + j + 1
            l4 = (k+1)*(k+2)/2 + j + 1
            a(l4) = (j+0.5_wp*k+0.125_wp/(2.0*j+k+1.0))*a(l3)             &
...
!                NM --- Highest order computed
      ...
      if ( x<=8.0_wp ) then
         ...
         do l = 0 , 1
            ...
            r = 1.0_wp
            do k = 1 , k0
               r = 0.125_wp*r*(vt-(2.0*k-1.0)**2)/(k*x)
               bkl = bkl + r
            enddo
            Bk(l) = a0*bkl
         enddo
...
!                          c0, c2,...
      ...
      do k = nm , 1 , -1
         ...
         if ( abs(f)>abs(Ck(k+1)) ) then
            ...
            fl = Ck(k+1)
            f1 = 1.0_wp
            f2 = 0.25_wp*((m+ip)*(m+ip+1.0)-Cv+cs)/(m+1.0)*f1
            Ck(1) = f1
            if ( kb==1 ) then
               fs = f2
            elseif ( kb==2 ) then
               Ck(2) = f2
               fs = 0.125_wp*(((m+ip+2.0)*(m+ip+3.0)-Cv+cs)*f2-cs*f1)    &
                  & /(m+2.0)
            else
               Ck(2) = f2
               do j = 3 , kb + 1
                  f = 0.25_wp*(((2.0*j+m+ip-4.0)*(2.0*j+m+ip-3.0)-Cv+cs) &
...
!       Output:  PMV --- Pmv(x)
      ...
      real(wp) c0 , eps , pa , Pmv , pss , psv , pv0 ,&
...
!                HEI --- kei'x
      ...
      if ( abs(x)<10.0_wp ) then
         ...
         r = -0.25_wp*x*x2
         gs = 1.5_wp
         Her = 1.5_wp*r - Ber/x - (log(x/2.d0)+gamma)*Der + 0.25*pi*Dei
         do m = 1 , 60
            r = -0.25_wp*r/m/(m+1.0_wp)/(2.0_wp*m+1.0_wp)**2*x4
         ...
         r = 0.5_wp*x
         gs = 1.0_wp
         Hei = 0.5_wp*x - Bei/x - (log(x/2.d0)+gamma)*Dei - 0.25*pi*Der
         do m = 1 , 60
...
!                VM --- Highest order computed
      ...
      if ( x<=18.0 ) then
         ...
         r = 1.0_wp
         do k = 1 , k0
            r = -0.125_wp*r*(vt-(2.0_wp*k-1.0_wp)**2.0)/(k*x)
            sum = sum + r
         enddo
         bi0 = ca*sum
      endif
      ...
      if ( x>9.0_wp ) then
         ...
         r = 1.0_wp
         do k = 1 , k0
            r = 0.125_wp*r*(vt-(2.0*k-1.0)**2.0)/(k*x)
            sum = sum + r
         enddo
         bk0 = cb*sum
...
!                VK2 --- K2/3(x)
      ...
      if ( x<=18.0 ) then
         ...
         do l = 1 , 2
            ...
            r = 1.0_wp
            do k = 1 , k0
               r = -0.125_wp*r*(vv-(2.0_wp*k-1.0_wp)**2.0)/(k*x)
               vsl = vsl + r
            enddo
            if ( l==1 ) Vi1 = c0*vsl
            if ( l==2 ) Vi2 = c0*vsl
      ...
      if ( x<=9.0_wp ) then
         ...
         do l = 1 , 2
            ...
            r = 1.0_wp
            do k = 1 , k0
               r = 0.125_wp*r*(vv-(2.0*k-1.0)**2.0)/(k*x)
               sum = sum + r
            enddo
            if ( l==1 ) Vk1 = c0*sum
            if ( l==2 ) Vk2 = c0*sum
...
!                VM --- Highest order computed
      ...
      if ( a0<18.0 ) then
         ...
         cr = (1.0_wp,0.0_wp)
         do k = 1 , k0
            cr = -0.125_wp*cr*(vt-(2.0_wp*k-1.0_wp)**2.0)/(k*z1)
            cs = cs + cr
         enddo
         cbi0 = ca*cs
      endif
...
!       Output:  GA --- Г(x)
      ...
      if ( x==int(x) .and. x>0.0 ) then
         ...
         do k = 2 , m1
            Ga = Ga*k
         enddo
      elseif ( x+.5_wp==int(x+0.5_wp) .and. x>0.0 ) then
         m = int(x)
         Ga = sqrtpi
         do k = 1 , m
...
!                NM --- Highest order computed
      ...
      if ( a0<=9.0 ) then
         ...
         do l = 0 , 1
            ...
            cr = (1.0_wp,0.0_wp)
            do k = 1 , k0
               cr = 0.125_wp*cr*(vt-(2.0*k-1.0)**2)/(k*z1)
               cbkl = cbkl + cr
            enddo
            Cbk(l) = ca0*cbkl
         enddo
